...
def create_dataset(soundfile_dict, vowels, num_mfccs):
    ...
    for i, vowel in enumerate(vowels):
        for filename in soundfile_dict[vowel]:
            ...
            mfcc = librosa.feature.mfcc(y=utterance, sr=16000, n_mfcc=num_mfccs, n_fft=512, win_length=400, hop_length=160)
            erm = mfcc[:, (mfcc.shape[1]//2)]
            dataset[index, 1:] = erm.transpose()
            dataset[index, 0] = i
            index += 1

    # for i, _ in enumerate(vowels):
    #     matrix = dataset[dataset[:, 0] == i, 1:]
    #
    #     for feature in range(num_mfccs):
    #         mu = np.mean(matrix[:, feature])
    #         sigma = np.std(matrix[:, feature])
    #         matrix[:, feature] = [(x-mu)/sigma for x in matrix[:, feature]]
    #     dataset[dataset[:, 0] == i, 1:] = matrix
    matrix = dataset[:, 1:]
